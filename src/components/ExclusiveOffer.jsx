...
function ExclusiveOffer() {
  return (
    <div className='flex flex-col items-center  px-6 md:px-16 lg:px-24 pt-20 pb-30'>
      <div className='flex flex-row items-center justify-between w-full md:flex-row'>
        <Title align='left' title="Exclusive Offer" subTitle="Take advantage of our exclusive offers and get the best deals on your next trip." />
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12'>
        {
            exclusiveOffers.map((item, index) => (
                <div key={item._id} className='group relative flex flex-col items-start justify-between gap-1 pt-12 md:pt-18 px-4 rounded-xl text-white bg-no-repeat bg-cover bg-center ' style={{ backgroundImage: `url(${item.image})` }}>
                    ...
                    <div >
                        <p className='text-2xl font-medium font-playpair'>{item.title}</p>

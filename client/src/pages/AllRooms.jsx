      ...
      <div>
        <div className='flex flex-col items-start text-left'>
          ...
          <p className='text-sm md:text-base mt-2 text-gray-500/90 max-w-174'>
            Take advantage of our exclusive offers and special packages to enhance your stay and create memorable moments.
          </p>
        </div>
          ...
          <div className={`${openFilters ? 'h-auto' : 'h-0 lg:h-auto'} overflow-hidden transition-all duration-700`}>
            <div className='px-5 py-5'>
              ...
              {roomTypes.map((roomType, index) => (
                <CheckBox key={index} label={roomType} index={index} selected={selectedRoomTypes.includes(roomType)} onChange={handleRoomTypeChange} />
              ))}
            </div>
            ...
            <div className='px-5 py-5'>
              ...
              {priceRanges.map((price, index) => (
                <CheckBox key={index} label={`$ ${price}`} index={index + 4} selected={selectedPriceRanges.includes(`$ ${price}`)} onChange={handlePriceChange} />
              ))}
            </div>
            ...
            <div className='px-5 py-5 pb-7'>
              ...
              {sortOptions.map((option, index) => (
                <RadioButton key={index} label={option} index={index} selected={selectedSortOption === option} onChange={handleSortChange} />
              ))}
            </div>
          </div>
        </div>
